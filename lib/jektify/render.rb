module Jektify
      class Render
        ...
        def content(app_name, app_root_config, spotify_embed_category, spotify_embed_url, spotify_embed_theme)
          ...
          if app_root_config["spotify"].nil?
            jektify__user_see = ""
              ...
              jektify__user_see = "<div class=\"jektify__user\"><a href=\"https://open.spotify.com/user/#{app_root_config["spotify"]["user"]}\" target=\"_blank\" class=\"jektify__userlink jektify__userlink--#{spotify_embed_theme} jektify__userlink--custom\" title=\"#{app_root_config["spotify"]["text"]}\"><span class=\"jektify__usertext jektify__usertext--#{spotify_embed_theme} jektify__usertext--custom\">#{app_root_config["spotify"]["text"]}</span></a></div>" if !app_root_config["spotify"]["user"].nil? && !app_root_config["spotify"]["text"].nil?
          end

         # Render template if enable == true
         if app_root_config["enable"] == true
              %(<dl class="jektify jektify--#{spotify_embed_theme} jektify--custom">
                <dt class="jektify__header jektify__header--#{spotify_embed_theme} jektify__header--custom"><a class="jektify__brand jektify__brand--#{spotify_embed_theme} jektify__brand--custom" href="https://#{app_name}.github.io" target="_blank">#{app_name}</a><i class="jektify__year jektify__year--#{spotify_embed_theme} jektify__year--custom">&copy;&nbsp;#{$jektify__year.strftime("%Y")}&nbsp;</i>#{jektify__toggle_button}</dt>
